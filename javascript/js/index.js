...
console.log("nums has %d elements", nums.length);
...
let courses = ["info 343", "info 360", "engl 270"];
console.log("my courses are:", courses);

//TODO: write the last element in the array to the console
console.log("my last course is", courses[courses.length - 1]);
...
//let key= "firstName";
//console.log(player[key]);  //player.key will be undefined
...
let courses2 = [
    ...
    class: "ENGL 270",
    section: "A",
    teachers: ["John Webster"]
    },
    {
    class: "INFO 360", 
    section: "A", 
    teachers: ["Jaime Synder", "Beck Tench"]
    },
    {
    class: "INFO 343", 
    section: "D",
    teachers: ["David Stearns", "Jessica Basa"]
    }
];
...
function levelUp(thePlayer) {
    ...
    thePlayer.level++;
    thePlayer.totalPoints += Math.ceil((thePlayer.totalPoints * (0.1 * thePlayer.level)));
    return thePlayer;
}
...
function randomIntegers(amount, max) {
    ...
    for (let i = 0; i < amount; i++) {
        ranNums.push(Math.round(Math.random() * max));
    }
    return ranNums;
}
